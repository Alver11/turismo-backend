...
class TrainOpenAIModelJob implements ShouldQueue
{
    ...
    public function handle(GenerateDatasetService $datasetService)
    {
        ...
        $client = Http::baseUrl(config('services.openai.base_url'))
            ->withToken(config('services.openai.api_key'))
            ->timeout(60);

        $uploadResponse = $client
            ->attach('file', file_get_contents($filePath), 'dataset_chat.jsonl')
            ->post('files', [
                'purpose' => 'fine-tune',
            ]);
        ...
        // Entrenar el modelo
        $trainResponse = $client->asJson()->post('fine_tuning/jobs', [
            'training_file' => $fileId,
            'model' => config('services.openai.fine_tuning_model'),
        ]);
